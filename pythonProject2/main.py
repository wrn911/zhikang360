...
if not w3.is_connected():
    raise Exception("Unable to connect to the Ethereum node. Please check the RPC endpoint")

# 加载合约ABI
with open("E:\\zhikang360\\pythonProject2\\abi3.json", "r") as file:
    abi = json.load(file)
...
# 构建mintNFT交易
metafile = "https://ipfs.io/ipfs/bafkreidbdpvtqid5bfj3hwieowbojva76xxnurk63ob3h4qkgyrrisydgq"
...
print(f"The transaction has been sent. Transaction hash: {tx_hash.hex()}")

# 等待交易确认
receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
print(f"The transaction has been confirmed. Block number: {receipt.blockNumber}")
