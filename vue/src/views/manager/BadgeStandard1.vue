<template>
  <div>
    ...
    <!-- AI生成图片预览对话框 -->
    <el-dialog title="AI生成图片预览" :visible.sync="aiPreviewVisible" width="40%">
      <div style="text-align: center;">
        <img :src="aiGeneratedUrl" alt="AI生成图片" style="max-width: 100%; max-height: 300px;" />
      </div>
      <div style="margin-top: 20px;">
        <el-button type="primary" @click="confirmAIImage">确认使用</el-button>
        <el-button type="info" @click="regenerateAIImage">重新生成</el-button>
        <el-button @click="aiPreviewVisible = false">取消</el-button>
      </div>
      <div style="margin-top: 20px;">
        <el-input v-model="aiSuggestion" placeholder="如果不满意，请输入建议"></el-input>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleAIGenerate(row) {
      this.currentAIGeneratingRow = row
      this.$request.post('/py/create', { id: row.id }).then(res => {
        if (res.code === '200') {
          this.aiGeneratedUrl = res.data.url
    ...
    // 确认使用AI生成的图片
    confirmAIImage() {
      this.currentAIGeneratingRow.url = this.aiGeneratedUrl
      this.$message.success('确认使用AI生成的图片')
      this.aiPreviewVisible = false
      // 调用Java后端的update接口保存图片
      this.$request({
        url: '/badgeStandard/update',
        method: 'PUT',
        data: this.currentAIGeneratingRow
      }).then(res => {
        if (res.code === '200') {
          this.$message.success('保存成功')
        } else {
    ...
    regenerateAIImage() {
      if (this.aiSuggestion) {
        // 如果有建议，将建议作为参数传递给后端
        this.$request.post('/py/create', { id: this.currentAIGeneratingRow.id, suggestion: this.aiSuggestion }).then(res => {
          if (res.code === '200') {
            this.aiGeneratedUrl = res.data.url
            this.$message.success('AI重新生成图片成功')
          } else {
            this.$message.error(res.msg)
          }
        })
        ...
        // 如果没有建议，直接重新生成
        this.$request.post('/py/create', { id: this.currentAIGeneratingRow.id }).then(res => {
          if (res.code === '200') {
            this.aiGeneratedUrl = res.data.url
            this.$message.success('AI重新生成图片成功')
          } else {
